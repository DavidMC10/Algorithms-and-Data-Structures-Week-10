...
int HashTable<T>::findPosLinear(const std::string & key) const {
	...
	// Q: handle the scenario where the table is full and the key does not exist in the table
	// A: we can simply count the number of iterations of this loop and ensure it is < table.length()
	int count = 0;
	while (isActive(pos) && table[pos].key != key && count < table.length()) {
