...
bool HashTable<T>::find(std::string key, T & val) const {
	...
	int pos = hash(key);							// hash the key
	Array<LinkedList<HashEntry>> temp = table;		// Create temp table based on Array of linkedlists
	LinkedList<HashEntry> & list = temp[pos];	    // select correct list								
	int i = list.find(HashEntry(key));				// determine position of HashEntry in list

	if (i != -1) {
		val = list.get(i).element;                  // copy value to reference parameter
		return true;
	}
